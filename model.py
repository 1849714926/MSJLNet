...
from IPM import IPM
from JSLM import JSLM
from resnet import resnet50, resnet18
from fsa_methods import pha_unwrapping
...
class embed_net(nn.Module):
    def __init__(self, class_num, datasets, arch='resnet50', ipm=False, jslm=False, pool_dim=2048, part_num=10):
        ...
        if self.dataset == 'regdb':
            # self.MFA1 = MFA(in_channels=256)
            # self.MFA2 = MFA(in_channels=512)
        ...
        if self.jslm:
            ...
            self.JSLM = JSLM(part_num=self.part_num, dim=pool_dim)
        else:
            self.part_num = 0
    ...
    def forward(self, x1_1, x1_2, x2_1, x2_2, modal=0):
        if modal == 0:
            # x1_1 = x1_1 + 0.8 * pha_unwrapping(x1_1)
            # x1_2 = x1_2 + 0.8 * pha_unwrapping(x1_2)
            # x2_1 = x2_1 + 0.8 * pha_unwrapping(x2_1)
            # x2_2 = x1_2 + 0.8 * pha_unwrapping(x2_2)
        ...
        if self.gm_pool == 'on':
            ...
            if self.jslm:
                xp = x_pool.unsqueeze(-1).unsqueeze(-1)
        else:
            xp = self.avgpool(x)
        ...
        if self.jslm:
            ...
            for i in range(self.part_num):
                for j in range(i + 1, self.part_num):
                    loss_ch += ((((sim[:, i] - sim[:, j]) ** 2).sum(dim=1) / (24 * 12)) ** 0.5).sum()
            loss_ch = - loss_ch / (xp.size(0) * self.part_num * (self.part_num - 1) / 2)
            loss_ch *= 0.5

        if self.training:
            if self.jslm:
                return x_pool, self.classifier(feat), feats, loss_ch, self.classifier_s(self.bottleneck_s(feats))
            else:
            ...
            if self.jslm:
                ...
                return self.l2norm(feat), self.l2norm(self.bottleneck_s(feat))
